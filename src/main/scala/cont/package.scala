...
package object cont {
  ...
  implicit class CC[A](a: A) {
    def cc[B](f: A => B) = f(a)
  }

  def reset(expr: Any): Any = macro resetImpl

  def resetImpl(c: scala.reflect.macros.whitebox.Context)(expr: c.Expr[Any]): c.universe.Tree = {
    import c.universe._
	...
	case q"$e; ..$es" =>
	  es match {
	    case Nil =>
	      e match {
		case q"$f[..$_](..$args)" =>
		  application(args)(as => apply(q"$f(..$as)", name)(cont))
		case q"val $x: $t = $a" => transform(a, x)(cont)
		case _ => apply(e, name)(cont)
	      }
	    case _ =>
	      transform(e)(_ => transform(q"{ ..$es }")(cont))
	  }
	case _ => transform(q"{ $expr }")(cont)
      }

    transform(c.untypecheck(expr.tree))(x => x)
  }
